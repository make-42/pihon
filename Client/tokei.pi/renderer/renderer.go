...
func DrawVerticalLine(startX int, startY, length int, v int) {
	for i := 0; i < length; i++ {
		DrawPixel(startX, startY+i, v)
	}
}

func DrawWidgetWindow(x, y, w, h int) {
	for i := 0; i < ScreenCount; i++ {
		DrawVerticalLine(x, y+1, h-2, 1)
		DrawVerticalLine(x+w, y+1, h-2, 1)
		DrawHorizontalLine(x+1, y, w-2, 1)
		DrawHorizontalLine(x+1, y+h, w-2, 1)
	}
}
...
func DrawSunsetIcon(x, y int) {
	DrawHorizontalLine(3+x, 0+y, 2, 1)
	DrawHorizontalLine(2+x, 1+y, 4, 1)
	DrawHorizontalLine(2+x, 2+y, 4, 1)
	DrawHorizontalLine(3+x, 3+y, 2, 1)
	DrawHorizontalLine(2+x, 5+y, 1, 1)
	DrawHorizontalLine(5+x, 5+y, 1, 1)
	DrawHorizontalLine(3+x, 6+y, 2, 1)
}

func DrawSunriseIcon(x, y int) {
	DrawHorizontalLine(3+x, 0+y, 2, 1)
	DrawHorizontalLine(2+x, 1+y, 1, 1)
	DrawHorizontalLine(5+x, 1+y, 1, 1)
	DrawHorizontalLine(3+x, 3+y, 2, 1)
	DrawHorizontalLine(2+x, 4+y, 4, 1)
	DrawHorizontalLine(2+x, 5+y, 4, 1)
	DrawHorizontalLine(3+x, 6+y, 2, 1)
}
...
func UpdateFrameBuffersToFiles(frameBuffer *image.Gray, bufferIndex int, sequenceIndex int) {
	f, _ := os.Create("./virtual-screens/" + fmt.Sprintf("%04d", sequenceIndex) + "." + strconv.Itoa(bufferIndex) + "img.png")
